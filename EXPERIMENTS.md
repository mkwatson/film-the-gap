...
## E24 — Mission-bound capture timing

**Frozen claim:** A one-phone contributor flow can add honest evidence that a recording followed a specific filming request without claiming that a browser timestamp, file digest, model, or video proves identity or product authenticity.

**Pass criteria:** Every mission receives an unpredictable human-friendly phrase; a contributor can speak it or show it in the same uninterrupted product video without a second device; the multimodal proposal reports exact-phrase detection separately from the product claim; a browser publication payload cannot assert verification; authorized imports stay preexisting; unavailable or negative model checks degrade to contributor-attested timing; the timing label survives Durable Object state, Site Tool output, D1 indexing, and later-shopper reuse; and every UI/README/submission claim states the narrow boundary.

**Result:** **graduated locally; real physical-model verification remains pending.** Mission creation generates a random two-word/two-digit phrase such as `LIME ORBIT 47`. The phone brief asks the contributor to say it or show it written near the start while the product remains visible. AI SDK's typed video proposal has a separate `verified | not_detected | unclear` challenge result. The Cloudflare Worker derives `mission_challenge_verified`, `contributor_attested`, or `preexisting` from its stored proposal plus the contributor's provenance choice; the remote review schema contains no client-authored capture-timing field. D1 migration `0003_capture_timing.sql` carries that receipt into exact later-shopper reuse.
