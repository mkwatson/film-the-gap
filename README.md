...
9. The first shopper's answer changes live. The original product page and later matching shoppers now receive the reviewed recording and timestamp immediately instead of creating another filming mission. Playback needs no account, does not autoplay, begins at the cited interval, and is limited to 60 new signed tokens per UTC day.
...
Before any approved public mutation, `pnpm release:target-check` requires an exact clean commit, the new standalone Vercel link, credential-free public origins, the dedicated Worker name, matching non-placeholder D1 IDs, and a configured signed-playback origin identical to the Worker origin. See [DEPLOYMENT.md](DEPLOYMENT.md) for the required `WEBMCP_*` variables. The current ignored `.vercel` link and placeholders intentionally do not pass this guard.
