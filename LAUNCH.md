...
2. Create the dedicated video Gateway key privately with a `$5` quota, no refresh, no BYOK, and no auto top-up. Vercel currently lets the one request that crosses a key quota finish and its keys do not expire automatically, so this is a bounded soft limit rather than an absolute `$5` ceiling; record a manual revocation for the end of judging. Paste that key and the separate page-reader secret only into non-echoing SSH/dashboard prompts; no secret enters the agent transcript.
