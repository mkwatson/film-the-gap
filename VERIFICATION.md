...
The Vercel, Cloudflare, and GitHub CLIs authenticated successfully on August 28. The standalone Vercel project is linked to `mkwatson/film-the-gap`, automatically deploys `main` to the clean `film-the-gap.vercel.app` alias, and carries a `$5`, non-renewing AI Gateway project budget with no auto-top-up. Current Vercel guidance says one request crossing a quota may finish, so the runbook no longer describes that boundary as a mathematically hard cap. A live Gateway-native Exa search and Shopify Global Catalog/UCP query passed behind the production WAF ceiling.
