...
The interface uses Next.js `16.3.3` and React `19.2.8`. Vercel AI SDK `7.0.83` and AI Gateway provide the optional reviewed vision proposal, with a manual path remaining authoritative. Chrome-format model evals, 101 deterministic application/Workers tests, strict TypeScript, production builds, and a clean native Chrome runner verify tool churn and the complete buyer → host → room → merchant → room lifecycle while suppressing credentials from logs.
...
4. Open **Open phone host** in a second browser/device. The URL fragment is scrubbed after authentication. The host should see one normalized request serving eight test-agent decisions and no `$450`, buyer identity, cart credential, or continuation URL.
...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| WebMCP Leverage       | Tools appear only when live evidence makes them meaningful; real calls alter the shared page; the journey crosses to a second WebMCP merchant origin.                            | Nine native registrations across two origins; seven dynamic buyer definitions; schema/runtime validation; Chrome-format evals; credential-suppressed native journey. | Repeat the full prompt in the final ChatGPT build and capture DevTools/Site Tools evidence.             |
| Execution             | Coherent buyer page, separate phone host, camera and no-permission paths, privacy receipts, exact hold, exact UCP Cart, cancellation, reset, and preflight.                      | Revisioned/reconnect-safe Durable Objects; idempotency; strict TypeScript; 101 deterministic tests; two Worker dry-runs; production build.                           | Stable unprotected URLs, clean public clone, external comprehension test, and physical phone/item pass. |
...
| Creativity & Ambition | Private agents direct the physical world, evidence changes capability, and a separate open-web merchant owns the outcome.                                                        | Camera provenance + reviewed AI proposal + epistemic multicast + dynamic WebMCP + released UCP composition.                                                          | Make this causal chain—not vendor logos—the first 135 seconds of the video.                             |
...
| `2:27–2:40` | Three-second architecture card: ChatGPT ↔ native Site Tools ↔ Cloudflare evidence room ↔ human camera ↔ UCP merchant.                                | Name the load-bearing implementation once the audience already understands it.                        |
