...
The product contract is data-driven: a judge can enter an unseen product name, optional public URL, and observable question without adding code. Sources carry explicit rights, provenance, capture timing, continuity, confidence, and timestamp citations. Marketing text can identify a claim but cannot prove a physical behavior. Only reviewed, claim-specific evidence with usable rights changes the answer. Every filming mission now issues a random phrase the contributor can say or show in frame. Server-side model detection can label the recording `mission_challenge_verified`; otherwise it remains honestly `contributor_attested`, while imported clips remain `preexisting`. This is bounded evidence that a recording followed the request, not an identity, authenticity, or deepfake claim.
...
3. **Ambitious:** permitted source discovery finds what the web already shows before interrupting a human; WebMCP publishes only the missing proof to a privacy-minimized open board, any existing product owner can fulfill it, and the reviewed result becomes reusable evidence.
...
The flagship is now locally submission-coherent, not publicly submission-ready. Its strongest differentiated sequence is causal, permissionless, and compounding: ChatGPT encounters an evidence gap, creates the exact physical test through native WebMCP, explicitly publishes only the safe mission fields, a stranger who already owns the product finds and opens it through a second WebMCP surface, their reviewed timestamped video changes the first answer and available tools, and a fresh matching shopper reuses that exact citation without another mission. This sequence directly serves all four equally weighted criteria, with dynamic tool leverage also winning the first tie-break.
...
The autonomous implementation now reduces risk around those gates: the deployable Worker is evidence-only and separate from the known-good old release; case, public recruitment, upload, retention, analysis, discovery, and 30-day D1 reuse costs are bounded; each mission carries a one-phone freshness challenge whose verification cannot be asserted by the client; the release verifier checks commit identity, required bindings, live queries through both D1 contracts, all three page policies, hostile-origin denial, and durable read-back; and the exact standalone entrypoint passes the native stranger-supply/two-shopper loop locally. Until the public paid rehearsal passes, commerce, UCP, Lean, voice, livestreaming, rewards, or unrelated sponsor products have lower expected winning value than deployment, physical causality, and presentation.

The **public missing-proof board has now graduated** on the same D1 foundation. Publication is explicit; listings contain no shopper context or private token; their separate case-scoped public capability is revocable; they expire within 24 hours; fulfilled jobs disappear; the private handoff still works; and desktop/mobile/WebMCP/native acceptance all pass. It answers “how does anyone with the product receive the request?” without a store partnership or customer list, and gives judges a second entry point without becoming a marketplace story. The next work must prove this is immediately legible to a cold human and real on final public infrastructure. Rewards remain a post-hackathon growth mechanism: adding money now would introduce identity, fraud, payment, and moderation surfaces without improving the three-minute causal proof. UCP or commerce should return only as a downstream consumer of improved evidence, never as a second product story.
...
| ---------------------------------------------------------- | ------------------: | --------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Stable public buyer, room, and merchant origins            |           Very high |              No | The public MIT repository is live; keep the exact deploy/verify/rollback command ready and do not substitute another feature for permanent vendor authorization.       |
...
5. **Authoritative commerce:** the owned merchant now negotiates the released UCP `2026-08-25` Cart contract, issues real server-held carts from original inventory whose `$375` item plus `$48` fulfillment exactly matches the `$423` evidence-room quote, and exposes a private second-origin continuation. Shopify's separately tested preview still advertises `2026-04-08`; negotiate each merchant rather than pretending one version fits all. Graduate the owned path publicly, then add Shopify-native interoperability only if it strengthens rather than destabilizes this completed boundary.
