...
The autonomous implementation now reduces risk around those gates: the deployable Worker is evidence-only and separate from the known-good old release; case, public recruitment, upload, retention, analysis, discovery, and 30-day D1 reuse costs are bounded; the release verifier checks commit identity, required bindings, live queries through both D1 contracts, all three page policies, hostile-origin denial, and durable read-back; and the exact standalone entrypoint passes the native stranger-supply/two-shopper loop locally. Until the public paid rehearsal passes, commerce, UCP, Lean, voice, livestreaming, rewards, or unrelated sponsor products have lower expected winning value than deployment, physical causality, and presentation.
