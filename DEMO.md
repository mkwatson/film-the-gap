...
> ChatGPT can find the product, but not proof of the exact behavior—so it asks a person to film one missing observation; that reviewed citation changes the recommendation for this shopper and becomes reusable for the next one.
...
> Does the filled bottle stay leak-free when held upside down for 10 seconds?
...
The source shown before the test is the deployed app's own `/demo-product` page. Show **Claim only · not verified evidence**, the evidence-gap checklist, **0 reviewed videos**, and its two native Site Tools. After publication, return to that exact page and show the green reviewed-evidence card, timestamped source, and replacement Site Tool. This keeps the entire product-page → request → reviewed evidence → upgraded product-page story real and rights-clean without pretending to operate a store.

For the opening UCP canary, use a generic query and show the live provider/protocol/privacy receipt plus **Catalog context · not proof**. Mask or crop every returned third-party product name, merchant name, domain, image, and mark unless explicit permission has been documented. Do not replace the live call with a fixture. The rights-clean bottle remains the only physical product in the primary story.
...
UCP discovery prompt:

> Find a new plain stainless insulated bottle available in the US. My maximum price and shopping history are private. Send only a generic product query and country. Stop after the catalog results, and treat every listing claim as unverified context.
...
> Inspect the open filming requests and open the exact request for this bottle. Do not add or infer any shopper context.
...
Product-page return prompt:

> Return to the original product page and inspect its reviewed evidence. Tell me what the page can now show that it could not show before, with the exact citation.
...
## `2:58` visual and narration contract
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0:00–0:12` | On the root page, run the UCP discovery prompt. Show `search_product_catalog`, UCP/provider and privacy receipts, and **Catalog context · not proof**; mask all returned third-party identities.              | “ChatGPT can search real new products across merchants through UCP, while my budget and history never leave the conversation. But a catalog claim is still not proof.” |
| `0:12–0:24` | Owned bottle beside `/demo-product`; show **Claim only · not verified evidence**, then let its Site Tools inspect and open the exact prefilled case with **Not enough proof** and **Recommendation blocked**. | “This rights-clean page says leak resistant. Its Site Tools refuse to repeat that claim as an answer and open the exact ten-second question.”                          |
| `0:24–0:40` | ChatGPT searches reviewed D1 evidence, the product page, public video, and open web. Show the Browser Run receipt while the answer and recommendation stay blocked.                                           | “The network searches what already exists, but ordinary pages and public links stay leads—not proof.”                                                                  |
| `0:40–0:55` | ChatGPT creates and inspects the mission, tightens its acceptance boundary, then explicitly publishes only public fields. Show refinement disappearing after handoff.                                         | “When the gap remains, ChatGPT creates—and can tighten—the smallest useful filming job. Once handed off, the target locks.”                                            |
| `0:55–1:07` | Open `/missions`. ChatGPT inspects the board, opens this exact request, and reveals its bounded recorder.                                                                                                     | “No store integration or customer list is required. A person with access to the product can discover the request and help.”                                            |
| `1:07–1:27` | On an ordinary phone, show the mission and random phrase; say or show it while the product remains visible, then record the complete continuous test.                                                         | “No account and no app. The mission phrase checks that this take followed the request; it is not an authenticity claim.”                                               |
| `1:27–1:43` | Confirm analysis rights, show direct Stream upload, then the bounded Gateway proposal, cut-aware map, cited interval, and separate phrase receipt. Waiting may be cut.                                        | “Nothing reaches the model until the contributor confirms media rights. The AI proposes the proof interval, but it still cannot publish.”                              |
| `1:43–1:58` | Scrub the recording; review or correct material fields, choose publishing rights and reuse, check final human confirmation, and publish.                                                                      | “The contributor inspects the actual video, corrects the AI, chooses rights, and explicitly confirms the final evidence.”                                              |
| `1:58–2:16` | Buyer updates live: **Not enough proof → [ACTUAL RESULT]** and **Recommendation blocked → [ACTUAL RECOMMENDATION IMPACT]**. ChatGPT cites `[TIMESTAMP]`.                                                      | “That reviewed observation changes whether this product should be recommended for the exact requirement, with a moment anyone can verify.”                             |
| `2:16–2:46` | Return to `/demo-product`. Its missing-proof card is now the reviewed result; invoke its replacement tool, then open the citation for one beat on the signed viewer.                                          | “The original page has learned a fact its own copy could never prove. The raw video is private, but the cited moment remains verifiable.”                              |
| `2:46–2:58` | Show the minimal architecture card and arbitrary-product form. End on **Find real products. Film only what is missing. Reuse every reviewed fact.**                                                           | “Agents find products and evidence gaps; people film reality once; every future shopper gets a better, cited recommendation.”                                          |
...
Next.js product-evidence page on Vercel
   ↑ Shopify Global Catalog via UCP → real product variants
...
Cloudflare Durable Object ↔ contributor phone → Cloudflare Stream
   ↑ reviewed evidence            ↓ authorized MP4 → private + signed playback
...
1. **Primary real take:** final public origins, current ChatGPT in-app Browser, live UCP search with third-party identities masked, physical phone, owned object, issued phrase, real Stream upload, real Gateway proposal if available, explicit human review, and the resulting answer/recommendation change tool.
...
1. UCP discovery sends only the generic query and country, exposes selection only after results, rejects stale variants, and keeps catalog copy out of evidence;
2. the initial answer and question-scoped recommendation are blocked;
3. WebMCP creates a bounded mission, inspects and refines its exact revision, then creates the phone link;
4. the contributor capability survives reload after its URL fragment is scrubbed;
5. upload and model analysis remain blocked until explicit media-rights confirmation, then a complete cut-aware segment map and proposal reach human review while the synthetic no-phrase recording remains only contributor-attested;
6. the reviewer can correct **Supports** to **Contradicts**;
7. publication updates the buyer over WebSocket;
8. `inspect_answer_change` returns insufficient → contradicted, blocked → excluded, and the exact `00:01–00:11` citation;
9. a buyer reload reconnects to the same Durable Object state; and
10. a fresh matching case reuses that exact citation and exposes no filming-mission tool.

A separate native product-page check must also prove the first tool set (`inspect_product_claim`, `open_product_evidence_case`), real navigation into the strict handoff route, and the replacement tool set (`inspect_product_claim`, `inspect_reviewed_product_evidence`) after a reviewed index record appears.
...
- By `0:12`, a cold viewer understands that UCP finds real products but catalog claims are not proof.
- By `0:24`, native product-page Site Tools—not generic browsing or DOM automation—have inspected the gap and opened the exact case with its recommendation blocked.
...
- By `2:16`, the same buyer page shows both answer and question-scoped recommendation **before → after**, plus a newly available answer-change tool.
- ChatGPT's final statement includes the reviewed result and exact timestamp rather than a paraphrased marketing claim.
- By `2:48`, the original product page visibly reuses the same recording and exposes a new reviewed-evidence tool; the compounding future is demonstrated rather than narrated.
- The video never claims that a hash or mission phrase proves authenticity, AI proves truth, public video grants reuse rights, a self-described contributor label is verified, or everyone with a product can already be contacted.
