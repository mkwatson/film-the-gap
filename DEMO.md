...
Updated 2026-08-28 PT. The target cut is `2:58`: one physical product, one question the web cannot prove, one native WebMCP mission published to an open request board, one person with the product who finds it without a store/customer list, one real phone recording, one reviewed finding, one visibly changed ChatGPT answer, and a fresh shopper who reuses the same citation without another mission.
...
The source shown before the test is the deployed app's own `/demo-product` page. Show its visible “Claim only · not verified evidence” label and evidence-gap checklist. This keeps the entire product-page → Browser Run → filming-request story real and rights-clean without pretending to operate a store.
...
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0:00–0:10` | Owned bottle beside the rights-clean `/demo-product` page, then the buyer view. Highlight **Claim only · not verified evidence**, the exact gap, and **Not enough proof**.       | “This bottle says leak resistant. That does not prove it stays dry upside down for ten seconds—the question I actually care about.”                              |
| `0:10–0:27` | Send the primary prompt. ChatGPT inspects the case and calls native **Search existing product evidence**. Show the real Browser Run receipt while the answer stays insufficient. | “Site Tools search reviewed network evidence, product pages, public video, and the open web—but ordinary links are not automatically proof.”                     |
| `0:27–0:47` | ChatGPT creates the ten-second mission, bounded case, and explicitly publishes only its public fields. Show the tool frontier change.                                            | “When the gap remains, it creates the smallest useful filming job without publishing who asked, their history, preferences, conversation, or budget.”            |
| `0:47–1:00` | Open `/missions`. ChatGPT inspects the board, opens this exact request, and reveals its bounded recorder QR.                                                                     | “We do not need a store or customer list. Anyone with the product can discover the request and help.”                                                            |
| `1:00–1:20` | On an ordinary phone browser, show the exact mission and random phrase; say or show it while the product remains visible, then record the complete continuous test.              | “No account and no app. The mission phrase lets the network check that this take followed the request; it is not an authenticity claim.”                         |
| `1:20–1:38` | Select the video, show direct Stream upload, then the bounded proposal, cited interval, and separate phrase-detection receipt. Waiting may be cut.                               | “The clip uploads directly to Cloudflare Stream. Through Vercel AI Gateway, a video model proposes the result and checks the exact phrase. It cannot publish.”   |
| `1:38–1:55` | Scrub the clip; review or correct every material field, choose rights and reuse, then check the final human confirmation.                                                        | “The recorder reviews every material field, deliberately chooses publishing rights, and explicitly confirms the complete review. The AI cannot publish.”         |
| `1:55–2:08` | Publish. Buyer updates live to **Before: Not enough proof → After: [ACTUAL RESULT]**; `inspect_answer_change` appears.                                                           | “Publishing changes the first answer live and unlocks a new answer-change tool with the exact citation.”                                                         |
| `2:08–2:20` | ChatGPT calls the new tool and cites `[TIMESTAMP]` plus the reviewed observation.                                                                                                | “ChatGPT tells me exactly what changed, what the recording establishes, and where I can verify it.”                                                              |
| `2:20–2:43` | Send the network-reuse prompt. Show a fresh case become **[ACTUAL RESULT]**, the D1 receipt, same citation, and no mission.                                                      | “A second shopper asks the same thing. ChatGPT reuses the reviewed timestamp instead of interrupting another person. The answers improve as evidence compounds.” |
| `2:43–2:58` | Show the minimal architecture card and arbitrary-product form. End on **Search what exists. Film only what is missing. Never recapture known proof.**                            | “This works for any observable product question: search what people have shown, turn gaps into tiny requests, and record each useful fact once.”                 |
...
1. **Primary real take:** final public origins, current ChatGPT in-app Browser, physical phone, owned object, the issued phrase spoken or shown in frame, real Stream upload, real Gateway proposal if available, explicit human review, and the resulting answer-change tool.
2. **Owned source-page close-up:** `/demo-product` on the same final origin, its claim/gap labels, response Content Signal receipt, and the resulting Browser Run source receipt on the buyer page.
...
7. **Completed-mission fallback and fresh-shopper reuse:** after the real mission is published with reuse consent, open the exact same product URL and question in a new case, search, and capture the same Stream UID/timestamp resolving it with no mission creation. This is an honest replay of the earlier recording, not a new person, model call, or physical test.
...
4. upload and model-shaped proposal reach explicit human review, while the synthetic no-phrase clip remains only contributor-attested;
