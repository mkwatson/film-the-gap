...
- Maximize **coherent sponsor-product leverage**: use as many deep, orthogonal sponsor products—especially genuinely useful new releases—as materially strengthen the same product thesis. Prefer several load-bearing products across different layers over one isolated integration, but never add redundant vendors, demo exposition, or failure modes merely to increase the count. Every product must improve a rubric factor, judge-visible multiplier, execution confidence, or learning speed more than its coherence and reliability costs.
- New sponsor products receive an exploration bonus because they can increase novelty and expert delight, not an exemption from access, compatibility, fallback, and exact-runtime proof. Keep beta/gated capabilities off the only golden path.
